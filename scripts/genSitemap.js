...
const additionalPremiumTitles = Array.from({ length: 980 }, (_, i) => `Guía Premium Avanzada #${i + 21}: Especialización en Activos Digitales`);
const ALL_PREMIUM_GUIDES = [...PREMIUM_TOPICS, ...additionalPremiumTitles];
...
function generateSitemaps() {
    ...
    // 13. Premium Guides (The 1,000 Pilar Engine)
    const premiumUrls = ALL_PREMIUM_GUIDES.map(title => `/guias-premium/${slugify(title)}`);
    writeSitemap('sitemap-guias-premium.xml', premiumUrls, 1.0); // Maximum priority for pillar content
    sitemaps.push('sitemap-guias-premium.xml');
